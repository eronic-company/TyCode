    ...
    pub async fn run(
        ...
        'agent: for _iteration in 0..max_iterations {
            ...
            if response.tool_calls.is_empty() {
                // The model answered with text and requested no tools — the turn
                // is complete. Stop here and wait for the user instead of
                // re-prompting (which makes the model ramble "I am ready…").
                break 'agent;
            }
