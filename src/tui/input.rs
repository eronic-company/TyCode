        ...
        KeyCode::Enter => {
            ...
            match field_key.as_str() {
                ...
                k if k.starts_with("manage_keys_") => {
                    ...
                    let manage_state = KeyManageState::from_config(&prov, &app.config);
                    app.mode = AppMode::KeyManage(manage_state);
                }
                _ => {
                    let state = match &mut app.mode {
                        AppMode::Settings(s) => s,
...
fn handle_key_select_key(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        KeyCode::Esc => {
            // No key was chosen — default to first if none active yet
                    ...
                    KeyCode::Enter => {
                        if !buf.trim().is_empty() {
                            if let AppMode::KeyManage(s) = &mut app.mode {
                                s.add_step = Some(AddKeyStep::EnterKey {
                                    label: buf.trim().to_string(),
                                    key_buf: String::new(),
                                });
                            }
                        }
                ...
                match key.code {
                    ...
                    KeyCode::Enter => {
                        if !key_buf.trim().is_empty() {
                            ...
                            let duplicate = entries.iter().any(|e| e.label == label);
                            if duplicate {
                                app.set_status(&format!("Label '{label}' already exists — choose a different name"));
                            } else if entries.len() >= 100 {
                                app.set_status("Key limit reached (100 max)");
                            } else {
                                entries.push(crate::config::ApiKeyEntry {
                                    label: label.clone(),
                                    key: key_buf.trim().to_string(),
                                });
                                if let Err(e) = app.config.save() {
                                    app.set_status(&format!("Failed to save key: {e}"));
                                }
                            }
                            if let AppMode::KeyManage(s) = &mut app.mode {
